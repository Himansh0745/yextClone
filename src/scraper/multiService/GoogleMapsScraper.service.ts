  ...
  async scrapeGoogleMaps(query: string): Promise<LocationResponseDto[]> {
    ...
    try {
      ...
      try {
        ...
        // console.log(error);
        console.error('⚠️ Google Maps: Consent button not found or already dismissed.', (error as Error).message);
      }
      try {
        ...
        console.error('❌ Google Maps: No results found or page timed out.', (error as Error).message);
        return [];
      }
