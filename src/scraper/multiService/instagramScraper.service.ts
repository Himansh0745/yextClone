    ...
    try {
      ...
      for (const profile of profiles.slice(0, 2)) {
        ...
        try {
          ...
          // console.log(`⚠️ Could not deep scrape Instagram profile: ${profile!.link}`, e);
          console.error(`⚠️ Could not deep scrape Instagram profile: ${profile!.link}`, (e as Error).message);
        } finally {
          await newPage.close();
        }
      }
